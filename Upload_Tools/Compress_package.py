...
def copy(source, dst):
    if os.path.isfile(source):
        return shutil.copy(source, dst)
    else:
        return shutil.copytree(source, os.path.join(dst, os.path.basename(source)), symlinks=True)
...
class work:
    def __init__(self, path, agent, oa):
        ...
        self.workpath = os.path.realpath(path)
...
'''
    ...
    def __Compress(self, which):
        rp = os.path.realpath(which)
        if os.path.exists(rp):
            copy(os.path.join(self.package, 'readme.txt'), rp)
            os.chdir(rp)
            zip_dir('./dandantang', './dandantang.zip')
            zip_dir('./readme.txt', './dandantang.zip', mode='a')
        return os.chdir(self.dir)

    def __check_key(self, source, dst_path):
        ###创建dandantang.key文件
        os.chdir(self.workpath)
        md5key = os.path.join(dst_path, 'dandantang.key')
        p = subprocess.Popen(args=['cmd', '/c', 'FileCheck_MD5.exe', '-create', source, '-k', md5key], shell=True,
        ...
        if ret.split()[1] != 'successed':
            return False
        c = subprocess.Popen(args=['cmd', '/c', 'FileCheck_MD5.exe', '-check', source, '-k', md5key], shell=True,
                             stderr=subprocess.PIPE, stdout=subprocess.PIPE, stdin=subprocess.DEVNULL,
                             universal_newlines=True, startupinfo=self.fix, env=os.environ)
    ...
    def DB_Make_Package(self):
        ...
        s_dir = [os.path.join(self.package, i) for i in (
            'Center', 'Create_Npc', 'sql') if os.path.exists(os.path.join(self.package, i))]
        ...
        for p in s_dir:
            if p.endswith('sql'):
                for sqlfile in os.listdir(p):
                    copy(sqlfile, compress_path)
                self.sql(compress_path)
            copy(p, copypath)
        self.__check_key(copypath, compress_path)
    ...
    def FS_Make_Package(self):
        ...
        # os.makedirs(copypath)
        s_dir = [os.path.join(self.package, i) for i in (
            'Center', 'AreaRankServer', 'FightServer', 'sql') if os.path.exists(os.path.join(self.package, i))]
    ...
    def IIS_Make_Package(self, *, online=False, tp='all'):
        ...
        # os.makedirs(copypath)
        s_dir = [os.path.join(self.package, i) for i in (
            'Flash', 'AreaRankServer', 'Request', 'Resource') if os.path.exists(os.path.join(self.package, i))]
    ...
    def GS_Make_Package(self):
        ...
        # os.makedirs(copypath)
        s_dir = [os.path.join(self.package, i) for i in (
            'FightServer', 'AreaRankServer', 'Server', 'Server1') if os.path.exists(os.path.join(self.package, i))]
        compress_path = os.path.join(self.dir, r'gs\dandantang')
        if not s_dir:
            return False
        for p in s_dir:
            if p.endswith('Server') or p.endswith('Server1'):
                copy(p, os.path.join(copypath, 'Server1'))
                copy(p, os.path.join(copypath, 'Server2'))
            copy(p, copypath)
        self.__check_key(copypath, compress_path)
        return compress_path
    ...
    def GSIIS_Make_Package(self, *, online=False, tp='all'):
        ...
        s_dir = [os.path.join(self.package, i) for i in
                 ('Flash', 'AreaRankServer', 'Request', 'Resource', 'FightServer',
                  'Server', 'Server1') if os.path.exists(os.path.join(self.package, i))]
    ...
    def runner(self):
        db = self.DB_Make_Package()
        fs = self.FS_Make_Package()
        iis = self.GSIIS_Make_Package()
        for ps in os.listdir(self.dir):
            cpath = os.path.join(self.dir,ps)
